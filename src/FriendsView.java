...
public class FriendsView extends JPanel
{
	...
	public void rebuild()
	{
		...
		for (int i = 0; i < friendList.length; i++)
		{
			...
			System.out.print(f.getUsername() + ", Type: " + f.getType());
			...
			if (f.getType() == 2)
			{
				displayName.setText(Globals.ellipsize(f.getUsername(), 18));
				displayName.setForeground(new Color(0xFF0000));
			}

			JButton storyButton = null;
			ArrayList<Story> s = this.getFriendStory(f);
			if (s != null)
			{
				for (int j = 0; j < s.size(); j++)
				{
					byte[] storyBytes = SnapchatAPI.getStory(s.get(j));
					
					if (s.get(j).isImage())
					{
						try
						{
							InputStream in = new ByteArrayInputStream(storyBytes);
							BufferedImage bImageFromConvert = ImageIO.read(in);
							int w = bImageFromConvert.getWidth();
							int h = bImageFromConvert.getHeight();
							if (w < h)
							{
								bImageFromConvert = this.getScaledImage(bImageFromConvert, STORY_ICON_DIAMETER, (int)Math.floor(((double)STORY_ICON_DIAMETER / w * h)));
								h = bImageFromConvert.getHeight();
								bImageFromConvert = bImageFromConvert.getSubimage (0, (int)Math.floor((h - (double)STORY_ICON_DIAMETER) / 2.0), STORY_ICON_DIAMETER, (int)Math.floor((h - (double)STORY_ICON_DIAMETER) / 2.0) + STORY_ICON_DIAMETER);
							}
							else
							{
								bImageFromConvert = this.getScaledImage(bImageFromConvert, (int) Math.floor(((double)STORY_ICON_DIAMETER / h * w)), STORY_ICON_DIAMETER);
								w = bImageFromConvert.getWidth();
								bImageFromConvert = bImageFromConvert.getSubimage((int)Math.floor((w - (double)STORY_ICON_DIAMETER) / 2.0), 0, (int)Math.floor((w - (double)STORY_ICON_DIAMETER) / 2.0) + STORY_ICON_DIAMETER, STORY_ICON_DIAMETER);
							}
							ImageIcon storyIcon = new ImageIcon(overlay(bImageFromConvert, this.getScaledImage(this.getImage("/res/circular_overlay_unhighlighted.png"), STORY_ICON_DIAMETER, STORY_ICON_DIAMETER)));
							storyButton = new JButton(storyIcon);
							storyButton.setPreferredSize(new Dimension(STORY_ICON_DIAMETER, STORY_ICON_DIAMETER));
							storyButton.setBackground(new Color(0xFFFFFF));
							mainPanel.add(storyButton, BorderLayout.EAST);
						}
						catch (Exception e)
						{
							e.printStackTrace();
